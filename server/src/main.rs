...
async fn main() -> io::Result<()>  {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if let Ok((input, _)) = bincode::decode_from_slice::<PlayerInput, _>(&buf[..len], config) {
...
async fn game_loop(mut input_rx: Receiver<(SocketAddr, PlayerInput)>, snapshot_tx: Sender<GameWorld>) {
    ...
    loop {
        ...
        while let Ok((addr, input)) = input_rx.try_recv() {
            let id = *addr_to_id.entry(addr).or_insert_with(|| world.add_player(input.id));
